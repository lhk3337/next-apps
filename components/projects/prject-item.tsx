...
export default function ProjectItem(data: IProjectDataType) {
  ...
  return (
    <div className="project-card">
      ...
      <div className="p-3">
        ...
        <p>
          {prjInfo.startDateString} ~ {prjInfo.endDateString}
        </p>
        <span className="font-bold text-indigo-600 text-sm">
          Total : {getDateDiff(prjInfo.startDateString, prjInfo.endDateString)} Day
        </span>
        <div className="flex items-start mt-2">
          {prjInfo.tags.map((tag: Itag) => (
            <h1 className="px-2 py-1 mr-2 rounded-md bg-sky-200 dark:bg-sky-700 w-30" key={tag.id}>
              {tag.name}
            </h1>
          ))}
        </div>
